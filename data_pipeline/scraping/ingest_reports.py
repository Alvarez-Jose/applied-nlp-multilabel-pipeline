...
async def crawl_wafa_async(max_pages: int = 5, delay: float = 1.0) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Found {len(article_urls)} WAFA article URLs")
        
        # Scrape the articles
        articles = await scrape(article_urls)
        logger.info(f"Successfully scraped {len(articles)} WAFA articles")
...
def filter_existing_reports(articles: List[Dict[str, Any]], source_name: str) -> List[Report]:
    ...
    for article in articles:
        ...
        # Check for duplicates (local set lookup - fast!)
        ...
        try:
            ...
            new_count += 1
            existing_urls.add(url_str)  # Prevent duplicates in this batch
        except Exception as e:
            error_count += 1
...
async def ingest_source_async(source_name: str, max_pages: int = 5, delay: float = 1.0) -> int:
    ...
    try:
        add_reports(reports)
        logger.info(f"Successfully added {len(reports)} new reports from {source_name}")
        return len(reports)
    except Exception as e:
        logger.error(f"Failed to add reports to Google Sheets: {e}")
        return 0
